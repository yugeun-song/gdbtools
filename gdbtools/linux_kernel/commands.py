...
_PORT_BASE = {"aarch64": 1235, "riscv": 1236, "x86-64": 1234}
_PORT_SPAN = 16


def _tcp_state():
    """Ports that are LISTENING, and ports that already have a client.

    Read from /proc/net/tcp rather than probed, because a candidate must never be
    dialled: opening a TCP connection to a QEMU gdbstub disturbs its single-client
    state and can wedge the real attach that follows.  Same rule a launcher applies
    with `ss -ltnH` / `ss -tnH state established`."""
    listening, claimed = set(), set()
    for path in ("/proc/net/tcp", "/proc/net/tcp6"):
        try:
            ...
            pass
    return listening, claimed


def _qemu_stubs():
    """Every QEMU on this host that is serving a gdbstub: {port: {pid, kernel}}.

    A port number is not an identity -- it changes run to run, and two VMs of the
    SAME tree differ only by which one you happened to start first.  /proc/<pid>/cmdline
    carries the thing that does identify a VM: the kernel image it booted."""
    out = {}
    try:
    ...
    for pid in pids:
        ...
        for i, a in enumerate(argv):
            if a == "-gdb" and i + 1 < len(argv):
                m = re.search(r"tcp::?(\d+)", argv[i + 1])
                if m:
                    port = int(m.group(1))
            elif a == "-s" and port is None:
                port = 1234
            elif a == "-kernel" and i + 1 < len(argv):
                kernel = argv[i + 1]
        if port is not None:
            out[port] = {"pid": int(pid), "kernel": kernel}
    return out


def _kernel_root():
    """The tree this gdb session is debugging, for matching a VM against it."""
    root = _env("KERNEL_ROOT")
    if not root:
        vm = SESSION.vmlinux_path()
        root = os.path.dirname(vm) if vm else None
    try:
...
def _serves_tree(info, root):
    if not (info and info.get("kernel") and root):
        return False
    try:
        return os.path.realpath(info["kernel"]).startswith(root + os.sep)
    except Exception:
        return False


def _default_port_base():
    v = _env_int("PORT")
    if v is not None:
        return v
    out = execstr("show architecture") or ""
    for key, base in _PORT_BASE.items():
        if key in out:
            return base
    return 1234


class KConnect(gdb.Command):
    """kconnect [PORT | LO-HI | auto | list] : attach to this tree's QEMU gdbstub.

A front-end launch configuration pins one address, but every VM gets its own port, so
the pinned one goes stale the moment a second VM is up.  This finds the stub instead.

A stub is a candidate when it is LISTENING and has no client yet.  Candidates are read
from /proc/net/tcp and never dialled: opening a connection to a QEMU gdbstub disturbs
its single-client state and can wedge the real attach.  Candidates are then matched
against THIS tree by the kernel image the VM booted, read from /proc/<pid>/cmdline,
because a port number identifies nothing -- two VMs of the same tree differ only by
which one started first.

One match is taken.  Several matches are refused with the list printed, since guessing
which VM you meant would silently spend a debugging session on the wrong one.  Name the
port to settle it; a named port is always obeyed.

With no argument, or 'auto', the range comes from the target architecture, matching
Conventional: x86_64 1234, arm64 1235, riscv64 1236, sixteen ports each.  $GDBTOOLS_PORT
overrides the base.  'list' shows what is out there without attaching."""

    def __init__(self, name="kconnect"):
        super(KConnect, self).__init__(name, gdb.COMMAND_RUNNING)

    def invoke(self, arg, from_tty):
        # Deliberately NOT wrapped in @safe.  A front end that cannot see why the
        # attach failed shows an empty session with no error at all, which reads as
        # "the debugger is broken" rather than "no VM was there".  Refusals are
        # raised so the caller reports them; unexpected faults are raised too, as a
        # message rather than a traceback.
        try:
            self._run((arg or "").strip())
        except gdb.GdbError:
            raise
        except Exception as e:
            LOG.add("kconnect: %s: %s" % (type(e).__name__, e))
            raise gdb.GdbError("kconnect failed: %s: %s" % (type(e).__name__, e))

    def _run(self, arg):
        want_list = arg.lower() in ("list", "ls", "show")
        if want_list or arg.lower() in ("auto", "any", "-"):
            arg = ""
        lo = hi = pinned = None
        if arg:
            ...
            if m:
                ...
                lo = hi = pinned = int(arg)
            else:
                raise gdb.GdbError("usage: kconnect [PORT | LO-HI | auto | list]")
        if lo is None:
            lo = _default_port_base()
            hi = lo + _PORT_SPAN - 1

        listening, claimed = _tcp_state()
        stubs = _qemu_stubs()
        root = _kernel_root()
        rows = []
        for p in range(lo, hi + 1):
            if p not in listening:
                continue
            info = stubs.get(p) or {}
            rows.append({"port": p, "busy": p in claimed, "pid": info.get("pid"),
                         "kernel": info.get("kernel"), "mine": _serves_tree(info, root)})

        def row_line(r):
            return ("    :%-5d  %-6s  pid %-7s  %s  %s"
                    % (r["port"], "in use" if r["busy"] else "free",
                       r["pid"] if r["pid"] else "?",
                       "this tree " if r["mine"] else "other     ",
                       r["kernel"] or "(kernel image not readable)"))

        def refuse(headline, listed=()):
            raise gdb.GdbError("\n".join(["kconnect: " + headline]
                                         + [row_line(r) for r in listed]))

        if want_list:
            print("[%s] gdbstubs on %d-%d; this tree is %s" % (NAME, lo, hi, root or "unknown"))
            if not rows:
        ...
        if pinned is not None:
            r = next((x for x in rows if x["port"] == pinned), None)
            if r is None:
                refuse("nothing is listening on :%d. Start the VM first." % pinned)
            if r["busy"]:
                refuse(":%d already has a client attached." % pinned, [r])
            if r["kernel"] and not r["mine"]:
                print("[%s] kconnect: :%d booted %s, which is not under %s. Attaching anyway, since "
                      "you named the port." % (NAME, pinned, r["kernel"], root))
            print("[%s] kconnect: attaching to the gdbstub on :%d" % (NAME, pinned))
            gdb.execute("target remote 127.0.0.1:%d" % pinned)
            return

        free = [r for r in rows if not r["busy"]]
        busy = [r for r in rows if r["busy"]]
        mine = [r for r in free if r["mine"]]

        if not free:
            if busy:
                refuse("every gdbstub on %d-%d already has a client. Start another VM "
                       "or name a port." % (lo, hi), busy)
            refuse("nothing listening on %d-%d. Start the VM first, or name the "
                   "port it printed." % (lo, hi))

        # Ambiguity is refused rather than guessed.  Two VMs of the same tree look
        # identical from here except for a port number that means nothing, and
        # attaching to the wrong one wastes a debugging session silently.
        if len(mine) > 1:
            refuse("%d free VMs are running this same tree, so the one you meant cannot be known "
                   "from here. Name the port:" % len(mine), mine)
        if mine:
            pick = mine[0]
        elif len(free) == 1 and not free[0]["kernel"]:
            # Nothing to match against (the VM's cmdline was unreadable), and only one
            # candidate exists, so there is nothing to get wrong by taking it.
            pick = free[0]
            print("[%s] kconnect: :%d is the only free stub and its kernel image could not be read, "
                  "so it is taken unverified." % (NAME, pick["port"]))
        else:
            refuse("%d free gdbstub%s, and none of them booted a kernel under %s. Name the port "
                   "you want:" % (len(free), "" if len(free) == 1 else "s", root or "this tree"), free)

        print("[%s] kconnect: attaching to the gdbstub on :%d%s"
              % (NAME, pick["port"],
                 ("  (skipped, already in use: %s)" % ", ".join(str(r["port"]) for r in busy)) if busy else ""))
        gdb.execute("target remote 127.0.0.1:%d" % pick["port"])


# Underscore-prefixed helpers are part of this module's public surface for the
# rest of the package (`from .commands import *`), which would otherwise skip them.
__all__ = ['KEarly', 'P2V', 'V2P', 'KB', 'KW', 'KSr', 'KSregs', 'KFin', 'KCensus', 'KPt', 'KPgd', 'KPtHex', 'KOff', 'KX', 'MmView', 'KConnect']
